          ...
          <div className="absolute bottom-4 right-2 bg-background/90 backdrop-blur-sm rounded-lg p-2 z-[1000]">
            ...
            <div className="flex items-center gap-1">
              {currentConfig.legendItems.map((item) => (
                <div key={item.label} className="w-4 h-3 rounded-sm" style={{ backgroundColor: item.color }} title={item.label} />
              ))}
            </div>
      ...
      {mapMode === 'rain' && (
        <div className="p-4 space-y-3 border-t border-border/50">
          <div className="space-y-1">
            <Slider value={[currentFrameIndex]} min={0} max={Math.max(radarFrames.length - 1, 0)} step={1} onValueChange={(v) => { setCurrentFrameIndex(v[0]); setIsPlaying(false); }} className="w-full" />
          ...
          <div className="flex items-center justify-center gap-2">
            <Button variant="outline" size="icon" onClick={() => { setCurrentFrameIndex(0); setIsPlaying(false); }} disabled={radarFrames.length === 0}><SkipBack className="h-4 w-4" /></Button>
            <Button variant="default" size="icon" onClick={() => setIsPlaying(!isPlaying)} disabled={radarFrames.length === 0}>{isPlaying ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}</Button>
